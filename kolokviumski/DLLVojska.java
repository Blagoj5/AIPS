...
public class DLLVojska {
	...
	private static DLL<Integer> vojska(DLL<Integer> lista, int a, int b, int c, int d) {
		...
		while (current.element != a) {
			tmp.insertLast(current.element);
			current = current.succ;
		}
		
		while (current != null) {
			if (current.element == c) {
				while(current.element != d) {
					tmp.insertLast(current.element);
					current = current.succ;
				}
				tmp.insertLast(current.element); // GO STAVAM i d
				current = null;
			}
			else {
				current = current.succ;
		...
		while (current != null) {
			if (current.element == b) {
				current = current.succ; // ZA DA NE E B
				while (current.element != c) {
					tmp.insertLast(current.element);
					current = current.succ;
				}
				current = null;
		...
		while (current != null) {
			if (current.element == a) {
				while (current.element != b) {
					tmp.insertLast(current.element);
					current = current.succ;
				}
				tmp.insertLast(current.element); // ZA da go vnesam i b
				current = current.succ;
			}
			else if (current.element == d ) {
				current = current.succ; // ZA DA NE E d
				while (current != null) {
					tmp.insertLast(current.element);
					current = current.succ;
				}
